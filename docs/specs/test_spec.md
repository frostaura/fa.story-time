...
- Frontend line coverage floor: **70%** minimum for the unit-tested runtime surface (`src/frontend/src/components/**`, `src/frontend/src/services/**`, `src/frontend/src/pwa.ts`), hard-failed in CI via `cd src/frontend && npm run test:unit -- --coverage`.
