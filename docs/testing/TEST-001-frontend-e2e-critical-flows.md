...
- Use Playwright in two layers:
  ...
  - live-backend browser smoke flows for the highest-risk paths
...
- Browser E2E coverage includes UC-001 through UC-005 Playwright checks, live-backend smoke coverage, and responsive viewport assertions.

## Regression Notes
- `src/frontend/tests/e2e/quick-generate.e2e.test.tsx` runs against a real backend process to validate route wiring and payload contracts end-to-end.
- Playwright now combines mocked browser contract journeys with a real-backend smoke path so browser rendering and network wiring are both exercised.
